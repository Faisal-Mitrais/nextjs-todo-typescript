...
import { Table, Space, Tag, Button, FormInstance } from 'antd';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (data: Todo) => (
        <Space size="middle">
          <Button
            style={{ background: yellow.primary, borderColor: yellow.primary }}
            type="primary"
            icon={<EditOutlined />}
            shape="round"
            size="small"
            title="Edit"
            onClick={() =>
              form.setFieldsValue({ ...data, deadline: moment(data.deadline) })
            }
          />
          <Button
            danger
            ...
            size="small"
            title="Remove"
            onClick={() => dispatch(removeTodo(data.id ?? 0))}
          />
        </Space>
      ),
    },
